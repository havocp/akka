...
class ActorBasedExecutorSpec extends FlatSpec with ShouldMatchers {
  behavior of "ActorBasedExecutor"
  ...
  // repeat some tests a few times to help catch intermittent races
  ...
  it should "run tasks and shut down" in {
    repeat(5) {
      val executor = new ActorBasedExecutor()
      ...
      executor.shutdown() // stops new tasks from being submitted
  ...
  private def countThreadsUsed(maxThreads: Int)(implicit dispatcher: MessageDispatcher) = {
    liveTasks.reset

    val executor = dispatcher.newExecutor(maxThreads)
    val tasks = for (i ← 1 to 200)
  ...
  it should "not use up all the threads in a dispatcher" in {
    val corePoolSize = 5
    implicit val dispatcher = Dispatchers.newDispatcher("actorbasedexecutortest")
      .withNewThreadPoolWithLinkedBlockingQueueWithUnboundedCapacity
      .setCorePoolSize(corePoolSize)
      .build

    val n2 = countThreadsUsed(2)
    val nTooMany = countThreadsUsed(corePoolSize + 7)
    n2 should be(2)
    nTooMany should be(corePoolSize - 1)
  }
